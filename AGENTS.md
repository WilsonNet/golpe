...
| **Anands' art — the next big job?** | [`docs/anands-art.md`](docs/anands-art.md) — the brief: HD-2D characters (Octopath / DQ3 remake); fix the keying that eats 21% of her silhouette, then a Blender model that keeps her style, gated against her boards |
...
- **A sprite's draw scale comes from its body box, never its cell.** A sheet
  ...
  the collider centre, and `make-hero-art.py` measures feet-on-floor and figure
...
magazines** (the rifle and the machine gun ship at 4, the shotgun at 5, tuned in `tweakables/ranged.ts`): one loaded,
