...
const STORAGE_KEY = "vento.training.panel";
...
.vt-body { padding: 8px 10px 12px; display: grid; gap: 10px; }
.vt-section { display: grid; gap: 4px; }
.vt-legend { color: #8a90ad; text-transform: uppercase; font-size: 10px; letter-spacing: 1px; }
.vt-row { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.vt-row label { color: #b9bed4; }
.vt-panel select, .vt-panel input[type="number"] {
	background: #1b1f2e; color: #e8e8f0; border: 1px solid #3a3f55;
	border-radius: 3px; padding: 2px 4px; font: inherit; width: 96px;
}
.vt-panel select { width: 100%; }
.vt-buttons { display: flex; flex-wrap: wrap; gap: 4px; }
.vt-panel button {
	background: #262b3d; color: #e8e8f0; border: 1px solid #3a3f55;
	border-radius: 3px; padding: 3px 8px; font: inherit; cursor: pointer;
}
.vt-panel button:hover { background: #333a52; }
.vt-panel button.vt-live { background: #6b2530; border-color: #a33; }
.vt-hint { color: #7b8099; font-size: 10px; }
.vt-readout { display: grid; grid-template-columns: auto 1fr; gap: 2px 8px; }
.vt-readout dt { color: #8a90ad; }
.vt-readout dd { margin: 0; }
.vt-warn { color: #ffb454; }
.vt-ok { color: #8fd694; }
.vt-frame { border-top: 1px solid #2a2f42; padding-top: 6px; }
`;
...
function FrameData({ state }: { state: TrainingState }) {
	...
	if (!x) return <div className="vt-hint">No exchange yet.</div>;
	...
	return (
		<div className="vt-frame">
			<div className="vt-row">
				...
				<span className={x.outcome ? "vt-ok" : "vt-hint"}>
					{x.outcome ?? "whiff"}
					{x.damage > 0 ? ` · ${x.damage} dmg` : ""}
				</span>
			</div>
			<dl className="vt-readout">
...
function Fragmented({
	...
	return (
		<>
			<dt>{label}</dt>
			<dd className={warn ? "vt-warn" : undefined}>{value}</dd>
		</>
	);
}
...
export function TrainingPanel() {
	...
	useEffect(() => {
		const onPointerDown = (e: PointerEvent) => {
			...
			if (target?.closest(".vt-panel")) return;
	...
	if (!state || !config) {
		return (
			<div className="vt-panel">
				<style>{CSS}</style>
				<div className="vt-head">TRAINING</div>
				<div className="vt-body vt-hint">Connecting to the training room…</div>
			</div>
		);
	}
	...
	return (
		<div className="vt-panel">
			...
			>
				...
				<span className={state.connected ? "vt-ok" : "vt-warn"}>
			...
			{collapsed ? null : (
				<div className="vt-body">
					<div className="vt-section">
						<div className="vt-legend">Dummy</div>
						<select
						...
						<div className="vt-hint">{behaviourHint}</div>
						<div className="vt-row">
							<label htmlFor="vt-facing">Facing</label>
							<select
								id="vt-facing"
								value={config.facing}
								onChange={(e) =>
						...
						<div className="vt-row">
							<label htmlFor="vt-stance">Stance</label>
							<select
								id="vt-stance"
					...
					<div className="vt-section">
						<div className="vt-legend">Timing</div>
						{TIMINGS.map((t) => (
							<div className="vt-row" key={t.key}>
								<label htmlFor={`vt-${t.key}`} title={t.hint}>
									{t.label}
								</label>
								<input
									id={`vt-${t.key}`}
					...
					<div className="vt-section">
						<div className="vt-legend">Rules</div>
						{TOGGLES.map((t) => (
							<div className="vt-row" key={t.key}>
								<label htmlFor={`vt-${t.key}`}>{t.label}</label>
								<input
									id={`vt-${t.key}`}
									type="checkbox"
					...
					<div className="vt-section">
						<div className="vt-legend">Recording</div>
						<div className="vt-buttons">
							<button
								type="button"
								className={recording ? "vt-live" : undefined}
					...
					<div className="vt-section">
						<div className="vt-legend">Positions</div>
						<div className="vt-buttons">
					...
					<div className="vt-section">
						<div className="vt-legend">Live</div>
						<dl className="vt-readout">
							<Fragmented
								label="behaviour"
								value={
					...
					<div className="vt-section">
						<div className="vt-legend">Last exchange</div>
