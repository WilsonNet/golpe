 ...
 * and her dragon's are 352x176, while the generated heroes keep the collider
 * size exactly. A clip indexes a strip; a strip's cells are whatever this
 * table says they are.
 */
const SHEET_CELLS: Record<string, { w: number; h: number }> = {
	[TEX.dude]: { w: PLAYER_WIDTH, h: PLAYER_HEIGHT },
	[TEX.roll]: { w: ROLL_FRAME_W, h: PLAYER_HEIGHT },
	[TEX.anands]: { w: 168, h: 152 },
	[TEX["anands-roll"]]: { w: 168, h: 152 },
	[TEX["anands-dragon"]]: { w: 352, h: 176 },
	[TEX.jeffs]: { w: PLAYER_WIDTH, h: PLAYER_HEIGHT },
 ...
 * over the sheet's cell height. The generated heroes are 1 (cells equal the
 * collider); Anands' hand-drawn art is ~2.7x the collider, so her sprites
 * are drawn at 48/152.
 */
...
export async function loadAssets(): Promise<void> {
	...
	// the same dude strip — see that script for why it is 40px wide rather
	// than 32. Anands' hand-drawn art (see `scripts/make-anands-art.py`) and
...
function createHeroPoses(
	...
	// cover the same box the walk frames do. The generated heroes are 32x48;
