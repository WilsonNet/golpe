...
- **Five magazines per life — one more than the other guns.** The shotgun
  spends a whole blast (all five shells) to whiff, and the shell-by-shell
  reload is the longest silence in the game, so the extra rack is how a
...
`art/jeffs/jeffs.blend` is the source; `python3 scripts/make-hero-art.py
jeffs` renders it into `jeffs.png` + `jeffs.json` (a packed atlas of every
clip, both facings) and `jeffs-portrait.png`. He shares Lia's skeleton,
shader, camera and every clip's poses (`scripts/blender/sprite_rig.py`), so
he plays every clip the animation system picks — the sword's cuts driven by
move progress, the shotgun banded by aim — and only his look is his own
(`scripts/blender/jeffs_build.py`): the long coat skirt that ends above the
knee, coat tails on the cape bone that flap with the stride, a pump shotgun
stubbier than Lia's rifle, a sword on his back in the gun stance.
