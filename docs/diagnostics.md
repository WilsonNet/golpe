...
| `meleeSummary.knockdownsArmed` / `knockdownsPaidOnLanding` / `knockdownsRecovered` / `instaFalls` | the anti-air launch as one accounting: **armed = paid + recovered + insta-falled**. The floor collects the debt, a jump on the way down cancels it (the safe fall), or an airborne swing spikes it (the Insta Fall). `knockdowns` alone reads the same for a build that spikes the victim out of the air on the hit and kills the launch, which is the bug this mechanic exists to avoid. The battery's `safe fall` and `Insta Fall` rows drive the two new endings deterministically |
