"""Bootstrap Lia's Blender source file: model, toon materials, rig and clips.
...
The render side (`lia_render.py`, driven by `scripts/make-lia-art.py`) never
builds anything: it opens the .blend and renders whatever is in it. So an
artist's edit — a new hair spike, a recoloured tunic, a re-timed swing —
survives every re-render, and this script is only the starting point.

Conventions this file sets up (and that the render script relies on):
...
- The character faces +X in armature space. The armature object is yawed
  toward the camera (the 3/4 view every Toriyama sprite is drawn in).
- Every clip is an Action whose custom properties carry the game metadata:
  ...
  `lia_props` ("sword" | "rifle" | "none": which weapon objects are shown).
  Every keyframed frame of the action is one rendered sprite frame.
"""
...
def build_armature(collection):
    arm_data = bpy.data.armatures.new("LiaRig")
    arm = bpy.data.objects.new("LiaRig", arm_data)
...
def build_scene():
    ...
    scene.camera = cam
    # The raw render canvas; make-lia-art.py crops it to the frames' union.
...
def main():
    ...
    build_scene()
    col = bpy.data.collections.new("Lia")
    ...
    for name, fn, right, left, fps, loop, drive, props in CLIPS:
        ms = 0
        if drive == "move":
            key = "massive" if name == "massive" else name
            m = MOVES[key]
    ...
    arm.animation_data.action = bpy.data.actions["clip.idle"]
    arm.animation_data.action_slot = bpy.data.actions["clip.idle"].slots[0]
